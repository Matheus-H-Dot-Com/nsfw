...
anu = axios.get(`https://www.receitaws.com.br/v1/cnpj/${aris}`).then((res) => {
		...
		case 'spamcall':
			...
			break
				...
				case 'wiki':
					...
					tels = body.slice(6)	
                                        if (!isUser) return reply(mess.only.daftarB)				
					const wiki = JSON.parse(`https://api.duckduckgo.com/?q=${tels}&format=json&pretty=1&no_html=1&skip_disambig=1`, {method: 'get'})
					reply(wiki.Result)
					break	
			       case 'pesquisaen':
					if (args.length < 1) return reply('digite palavras-chave')
	    ...
	    case 'quando':
            ...
            break
				...
				case 'ttp':
					...
					break
                                case 'clearall':
					if (!isOwner) return reply('Limpando...')
					anu = await client.chats.all()
